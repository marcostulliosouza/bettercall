...
class ReceiveEquipmentsWindow(QDialog):
    """
    ...
    def __init__(self, invoiceId=None, deviceId=None, deviceType=None, parent=None):
        super(ReceiveEquipmentsWindow, self).__init__(parent)
        self.parent = parent
        ...
        self.clientSentEquipmentsTable = MyQTableWidget()


        # --> Botões #

        self.receiveButton = MyQPushButton("Receber")
        self.receiveButton.setEnabled(False)
        self.receiveButton.setFocusPolicy(Qt.NoFocus)
        ...
        buttonLayout.addWidget(self.receiveButton)
        ...
        windowLayout.addWidget(self.clientSentEquipmentsTable)
        windowLayout.addLayout(buttonLayout)
        windowLayout.addStretch(1)
        

        self.setLayout(windowLayout)
        self.setWindowIcon(QIcon(":/receive_equipment.png"))
        self.setWindowTitle("Receber Equipamentos")
        ...
        self.clientComboBox.currentIndexChanged.connect(self.populateClientSentEquipmentsTable)
        self.invoiceComboBox.currentIndexChanged.connect(self.unlockReceiveButton)
        self.invoiceComboBox.currentIndexChanged.connect(self.lockInvalidTableItem)
        self.clientSentEquipmentsTable.itemClicked.connect(self.toggleTableItem)
        self.receiveButton.clicked.connect(self.receiveEquipments)
        cancelButton.clicked.connect(self.close)


#####################################################################################################################################

    def receiveEquipments(self):
        """
        This method is called to receive the equipments from its owners.
        ...
        if not len(self.tableCheckedIndexes):
            ...
            messageBox.setText("Selecione os dispositivos que serão recebidos pelo Cliente.")
        ...
        for tableRow in self.tableCheckedIndexes:
            equipmentId = self.clientSentEquipmentsTable.item(tableRow, 1).data(Qt.UserRole)
        ...
        messageBox.setText("Confirmar recebimento dos Equipamentos abaixo para a nota " + self.invoiceComboBox.currentText() + "?" + devsString)
        ...
        if messageBox.clickedButton() == messageBox.button(QMessageBox.No):
            ...
            try:
                self.devicesList.receiveDevices(devsIdList, self.invoiceComboBox.itemData(self.invoiceComboBox.currentIndex()))
            ...
            self.populateClientSentEquipmentsTable()
            
            messageBox = QMessageBox()
            messageBox.setText("Recebimento de Equipamentos executado com sucesso!")
...
#####################################################################################################################################
    ...
    def lockInvalidTableItem(self):
        self.tableCheckedIndexes.clear()
        
        for i in range(0, self.clientSentEquipmentsTable.rowCount()):
            equipmentId = self.clientSentEquipmentsTable.item(i, 1).data(Qt.UserRole)
            device = self.devicesList.deviceFromId(equipmentId)

            

            self.clientSentEquipmentsTable.item(i, 0).setBackground(self.noBrush)
            self.clientSentEquipmentsTable.item(i, 1).setBackground(self.noBrush)
            self.clientSentEquipmentsTable.item(i, 2).setBackground(self.noBrush)
            self.clientSentEquipmentsTable.item(i, 3).setBackground(self.noBrush)
            
            if self.invoiceComboBox.itemData(self.invoiceComboBox.currentIndex()):

                if device.isInvoiceRelated(self.invoiceComboBox.itemData(self.invoiceComboBox.currentIndex())):

                    item = QTableWidgetItem()
                    self.clientSentEquipmentsTable.setItem(i, 0, item)
                    
                    self.clientSentEquipmentsTable.item(i, 0).setFlags(Qt.NoItemFlags)
                    self.clientSentEquipmentsTable.item(i, 1).setFlags(Qt.NoItemFlags)
                    self.clientSentEquipmentsTable.item(i, 2).setFlags(Qt.NoItemFlags)
                    self.clientSentEquipmentsTable.item(i, 3).setFlags(Qt.NoItemFlags)
                    ...
                    self.clientSentEquipmentsTable.item(i, 1).setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)
                    self.clientSentEquipmentsTable.item(i, 2).setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)
                    self.clientSentEquipmentsTable.item(i, 3).setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)
...
#####################################################################################################################################
    ...
    def populateClientSentEquipmentsTable(self):
        """
        ...
        filterFields.append("dis_status = '3'")
        ...
        self.clientSentEquipmentsTable.clearContents()
        self.clientSentEquipmentsTable.setRowCount(len(self.devicesList))
        ...
        for row, device in enumerate(self.devicesList):
            ...
            item = QTableWidgetItem()
            self.clientSentEquipmentsTable.setItem(row, 0, item)    
          
            # --> Identificação do Dispositivo #
            item = QTableWidgetItem(str(device.deviceId).zfill(6))
            item.setData(Qt.UserRole, device.deviceId)
            item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
            self.clientSentEquipmentsTable.setItem(row, 1, item)

            # --> Descrição #
            item = QTableWidgetItem(device.deviceDescription)
            self.clientSentEquipmentsTable.setItem(row, 2, item)

            # --> Código SAP #
            item = QTableWidgetItem(str(device.deviceSAPCode))
            item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
            self.clientSentEquipmentsTable.setItem(row, 3, item)

        if self.invoiceComboBox.currentIndex() > 0:
            self.lockInvalidTableItem()

            
#####################################################################################################################################

    def toggleTableItem(self, tableItem):
        if self.invoiceComboBox.currentIndex() < 1 or tableItem.flags() == Qt.NoItemFlags:
            return

        equipmentId = self.clientSentEquipmentsTable.item(tableItem.row(), 1).data(Qt.UserRole)
        device = self.devicesList.deviceFromId(equipmentId)
        if tableItem.row() in self.tableCheckedIndexes:
            self.tableCheckedIndexes.remove(tableItem.row())
            brush = self.noBrush
            self.clientSentEquipmentsTable.item(tableItem.row(), 0).setCheckState(Qt.Unchecked)

        else:
            self.tableCheckedIndexes.add(tableItem.row())
            brush = self.selectedBrush
            self.clientSentEquipmentsTable.item(tableItem.row(), 0).setCheckState(Qt.Checked)

            
        self.clientSentEquipmentsTable.item(tableItem.row(), 0).setBackground(brush)
        self.clientSentEquipmentsTable.item(tableItem.row(), 1).setBackground(brush)
        self.clientSentEquipmentsTable.item(tableItem.row(), 2).setBackground(brush)
        self.clientSentEquipmentsTable.item(tableItem.row(), 3).setBackground(brush)

#####################################################################################################################################


    def unlockReceiveButton(self):
        index = self.invoiceComboBox.currentIndex()
        if index > 0:
