...
class EditMaintenanceFormWindow(QDialog):
    """
    ...
    def __init__(self, maintenanceForm=None, parent=None):
        super(EditMaintenanceFormWindow, self).__init__(parent)
        ...
        self.formDescriptionField = MyQLineEdit(self.maintenanceForm.formDescription)
        ...
        self.setWindowTitle("Formulário - " + self.maintenanceForm.formDescription)
...
###############################################################################################################################################################################
    ...
    def saveForm(self):
        ...
        messageBox.setWindowTitle("Salvar Formulário?")
        messageBox.setText("Deseja salvar as alterações efetuadas neste formulário?")
        ...
        updateList = []
        deleteList = []
        for formItem in self.maintenanceForm.formItemsList:
            deleteList.append(formItem.itemId)
        ...
        for i in range(0, self.formItemsListLayout.count()):
            ...
            if self.formItemsListLayout.itemAt(i).widget().itemId:
                deleteList.remove(self.formItemsListLayout.itemAt(i).widget().itemId)
                updateList.append(
                    (self.formItemsListLayout.itemAt(i).widget().itemId,
                     self.formItemsListLayout.itemAt(i).widget().layout().itemAt(0).widget().text(),
                     self.formItemsListLayout.itemAt(i).widget().itemPosition)
                )

        # --> Try to update the form information
        try:
            self.maintenanceForm.updateFormItems(self.formDescriptionField.text(), self.parent.loggedUser["id"], insertList, updateList, deleteList)
        ...
        messageBox = QMessageBox()
        messageBox.setText("Formulário atualizado com sucesso.")
...
###############################################################################################################################################################################
    ...
    def populateItemsList(self):
        """
        This method loads the items inside the groupbox
        """
        ...
        for formItem in self.maintenanceForm.formItemsList:
            ...
            if formItem.itemPosition <= 0:
                moveUpButton.setEnabled(False)
                
            # --> Down Button
            moveDownButton = MyQDownPushButton(text="", parent=formItemFrame, itemsLayout=self.formItemsListLayout)
            
            if formItem.itemPosition >= (len(self.maintenanceForm.formItemsList) - 1):
                moveDownButton.setEnabled(False)
